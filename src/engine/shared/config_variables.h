...
MACRO_CONFIG_INT(BrMaxRequests, br_max_requests, 96, 0, 1000, CFGFLAG_SAVE | CFGFLAG_CLIENT, "Number of requests to use when refreshing server browser")
...
MACRO_CONFIG_INT(SndDevice, snd_device, -1, 0, 0, CFGFLAG_SAVE | CFGFLAG_CLIENT, "Audo device index")
...
MACRO_CONFIG_INT(GfxFinish, gfx_finish, 0, 0, 1, CFGFLAG_SAVE | CFGFLAG_CLIENT, "Wait until the GPU has finished the current frame before starting the new one")
...
MACRO_CONFIG_INT(GfxUIScale, gfx_ui_scale, -1, 0, 5, CFGFLAG_SAVE | CFGFLAG_CLIENT, "Client UI Scale, -1 = auto, 0 = 100%, 1 = 120%, 5 = 200%")
